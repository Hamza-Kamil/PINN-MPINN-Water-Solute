...
class C1:
    ...
    def net_res(self, t, z):
        ...
        fc=   rho*Kd*c_t + c_t + qc + mu1*theta*c  
...
class C3:
    ...
    def __init__(self, layers, LAA):
        ...
        self.c_ic = tf.fill(tf.shape(self.c_ic_pred), c_initial) #IC
        self.c_up = tf.fill(tf.shape(self.c_up_pred), c3_inlet) #Upper BC
    ...
    def net_res(self, t, z):
        ...
        c2 = self.net_c(X, self.weights_c, self.biases_c, self.A_c)
        theta, q = Richards.net_water(t, z, w, b, a)
        c1 = solute1.net_c1(t, z, wc1, bc1, ac1)
        c2_t = tf.gradients(theta*c2, t)[0]
        c2_z = tf.gradients(c2, z)[0]
        D = diffusion_term(theta,q,  thetaSvg, DL, Dw)
        qc = tf.gradients(q*c2-D*c2_z, z)[0]


        fc=    c2_t + qc - mu1*theta*c1 + mu2*theta*c2 
