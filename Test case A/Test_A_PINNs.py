...
class PINNs:
    ...
    def __init__(self, soil, size, layers, LAA):
        ...
        self.psi_pred, self.c_pred, self.psi_res_pred, self.c_res_pred = self.net_res(self.t_res_tf, self.z_res_tf)
    ...
    def net_coupled(self, X, weights, biases, A): # NN for psi
    ...
    def diffusion_term(self, theta, q, thetas, DL, Dw):
        return DL * tf.abs(q) + tf.pow(theta, 10.0/3) * Dw / tf.pow(thetas, 2.0)
...
def main(hydrus, size, num_layers, num_neurons, number_random, it):
   ...
   # the PINNs object
    model = PINNs(soil, size, layers, LAA= True) 
    #train
    model.train(it)
    ...
    pred_data = pd.DataFrame({'z': z_star.flatten(), 't': t_star.flatten(),
                         ...
                         'c_pred': c.flatten(),
                         'water_flux_pred': flux.flatten()})
    pred_data.to_csv(f"./Results/PINNsAdaptive_activation_predicted_data.csv")
    ...
    loss_data = pd.DataFrame({'total_loss': total_loss, 'res_loss': res_loss,
                        ...
                        'ic_c_loss': IC_R_loss,
                         'up_c_loss': up_R_loss,
                         'lb_c_loss': lb_R_loss })
    loss_data.to_csv(f"./Results/PINNsAdaptive_activation_loss_data.csv")

    loss_data = pd.DataFrame({'total_loss': total_loss })
    loss_data.to_csv(f"./Results/PINNs_Adaptive_activation_loss_data.csv")
...
main(soil, size, num_layers[3], num_neurons[0], number_random[0], 20000)