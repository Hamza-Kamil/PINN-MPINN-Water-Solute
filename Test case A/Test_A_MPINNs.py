...
class MPINNs:
    ...
    def __init__(self, soil, size, layers_psi, layers_c, LAA):
        ...
        self.weights_c, self.biases_c, self.A_c = self.initialize_NN(layers_c)
    ...
    def MPINNs_parameters(self):

        weights_psi_final = self.sess.run(self.weights_psi)
        biases_psi_final = self.sess.run(self.biases_psi)
        a_psi_final = self.sess.run(self.A_psi)   
        weights_c_final = self.sess.run(self.weights_psi)
        biases_c_final = self.sess.run(self.biases_psi)
        a_c_final = self.sess.run(self.A_c)
        return weights_psi_final, biases_psi_final, a_psi_final, weights_c_final, biases_c_final, a_c_final
...
def main(hydrus, size, num_layers_psi, num_neurons_psi, num_layers_c, num_neurons_c, number_random, it):
   ...
   # the PINNs object
    model = MPINNs(soil, size, layers_psi, layers_c, LAA= False) 
    #train
    model.train(it)
    ...
    weights_psi_final, biases_psi_final, a_psi_final, weights_c_final, biases_c_final, a_c_final = model.MPINNs_parameters()
    
    #### save the predicted data
    Parameters_data = pd.DataFrame({'W_psi': weights_psi_final, 'b_psi': biases_psi_final,
                         'a_psi':  a_psi_final,
                         'W_c': weights_c_final, 'b_c': biases_c_final,
                         'a_c':  a_c_final})
    Parameters_data.to_csv(f"./Results/MPINNs_NO_Adaptive_activation_Parameters_data.csv")
    ...
    loss_data.to_csv(f"./Results/MPINNs_NO_Adaptive_activation_loss_data.csv")    
                     
    pred_data = pd.DataFrame({'z': z_star.flatten(), 't': t_star.flatten(),
                         'psi': psi.flatten(),
                         'c': c.flatten(),
                         'q': flux.flatten()})
    pred_data.to_csv(f"./Results/MPINNs_Adaptive_activation_predicted_data.csv")
